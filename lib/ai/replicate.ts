...
export async function analyzeImage(imageData: string, mimeType: string) {
  try {
    ...
    const response = await fetch("https://api.replicate.com/v1/predictions", {
      ...
      body: JSON.stringify({
        ...
        input: {
          ...
          prompt:
            "Describe this image in detail. Count any people, assess engagement level (low/medium/high), describe activities, and mood.",
        },
      }),
    });
    ...
    const peopleCount = peopleMatch
      ...
      : text.includes("people") || text.includes("person")
        ? 1
        : 0;
    ...
    return {
      peopleCount,
      engagementLevel,
      activities: [aiOutput.substring(0, 200)],
      mood,
      insights: [
